...
读取 `<design-request>`，判断是否需要代码库探索。**满足以下任一条件则 spawn @researcher（串行，先于设计团队）：**

- design-request 包含具体文件路径或模块名引用
- 任务类型为 bug fix / refactor / feature extension（非全新项目）
- design-request 中未提供"代码库上下文"或"已有实现"信息
...
等待 @researcher 完成，将输出追加到 design-request 上下文中，再执行第一步。
