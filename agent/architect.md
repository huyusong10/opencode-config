...
**在进入目标反推之前，先召集设计团队进行多维度探索。**

#### 跳过条件（满足任一即跳过，直接进入阶段 3）

- 单文件修改，≤ 20 行
- 任务类型：配置更新、文档修改、typo 修复、简单 bug 修复
- 需求完全明确，无架构决策空间
- 可直接遵循既有模式，无需新设计

#### 触发流程

1. 组装 `<design-request>`，包含：项目背景与技术栈、当前阶段目标与成功标准、需求摘要（REQ-IDs）、已知约束与关键决策、研究结论（如有）

2. 调用 `@system-designer`，传入完整上下文：
...
## 项目背景
- **项目名称:** [PROJECT.md 中的名称]
- **技术栈:** [PROJECT.md 中的技术栈]
- **关键约束:** [PROJECT.md 中的约束]

## 当前阶段目标
- **阶段:** [ROADMAP.md 中的当前阶段编号与名称]
- **阶段目标:** [ROADMAP.md 中的目标描述]
- **成功标准:**
  - [成功标准1]
  - [成功标准2]

## 需求摘要
[REQUIREMENTS.md 中的相关需求，附 REQ-ID，聚焦本阶段]
...
3. 等待 `<design-synthesis>` 输出

4. 继续阶段 3，使用 synthesis 中的"**目标反推输入**"章节作为 Goal-Backward 的出发点
...
**前置条件（如已完成阶段 2.5）：**
从 `<design-synthesis>` 的"目标反推输入"章节获取：
- **预期可观察行为** → 用于步骤 2 (Derive Observable Truths)
- **关键产物** → 用于步骤 3 (Derive Required Artifacts)
- **必须避免的失败点** → 用于步骤 5 (Identify Critical Links)
...
**规划完成报告模板中已包含 `<system-review-request>` 标签，系统评审在每次规划完成后自动触发，不可跳过。**

规划上下文会自动路由到 `planning` 评审（arch + security + api + qa），聚焦规划质量和架构设计，不做代码执行类检查。

**收到 `<system-advisory>` 后：**
- P0 建议（安全/正确性）：强烈建议在规划中处理，再推进执行
- P1 建议（质量/架构）：视情况调整规划内容
- P2 建议（优化/创新）：可纳入 backlog，不阻塞执行
