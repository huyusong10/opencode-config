    ...
    for (const name of names) {
        ...
        if (target.instruction && selected(profile, "instructions")) {
            steps.push({
                target: name,
                asset: "instructions",
                source: `assets/instructions/${target.instruction.overlay}.md`,
...
function renderInstructions(target: Target) {
    ...
    const shared = readText(path.join(repo, "assets", "instructions", "base.md"))
    const overlay = readText(path.join(repo, "assets", "instructions", `${target.instruction.overlay}.md`))
    const header = [
        "<!--",
        "Generated by agentcfg. Do not edit this file directly.",
        "Source: assets/instructions/base.md + target overlay.",
        "-->",
    ].join("\n")

    return tpl
        .replace("{{generated_header}}", header)
        .replace("{{shared_instructions}}", shared.trim())
        .replace("{{target_overlay}}", overlay.trim())
        .trimEnd() + "\n"
}
...
function installCommand(spec: string, opts: Options) {
    ...
    for (const step of steps) {
        ...
        const backup = existsSync(dest) ? backupPath(backupId, dest) : undefined
...
function backupCommand(spec: string, opts: Options) {
    const manifest = readManifest()
    const names = new Set(expandTarget(spec))
    const entries = manifest.entries.filter((entry) => names.has(entry.target) && existsSync(entry.destination))
...
function restoreCommand(id: string, opts: Options) {
    ...
    for (const entry of index.entries) {
        ...
        if (existsSync(entry.destination)) copyExisting(entry.destination, backupPath(`pre-restore-${stamp()}`, entry.destination))
        rmSync(entry.destination, { recursive: true, force: true })
...
function validateCommand(silent = false) {
    ...
    for (const target of targets) {
        ...
        if (target.instruction) {
            const overlay = path.join(repo, "assets", "instructions", `${target.instruction.overlay}.md`)
            if (!existsSync(overlay)) throw new Error(`target ${target.name} overlay 不存在：${overlay}`)
        }
        for (const place of target.placements || []) {
