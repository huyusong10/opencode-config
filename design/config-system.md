...
`all` 是 target 聚合别名，展开为 profile 允许的目标集合；显式 target 若不在 profile targets 中，应拒绝执行。`--profile <name>` 与 `--profile=<name>` 表达同一个 profile 选择，`install.sh` bootstrap 入口也应按同一方式转发 profile。`--dry-run` 只输出计划，不修改目标目录或 manifest。
